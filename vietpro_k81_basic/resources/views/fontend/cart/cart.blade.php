				...
				<div class="row row-pb-md">
					<div class="col-md-10 col-md-offset-1">
						...
						<div class="product-cart">
							...
							<div class="one-eight text-center">
								<div class="display-tc">
									<input onchange="updateCart('{{ $row->rowId }}',this.value)" type="number" id="quantity" name="qty"
							...
							<div class="one-eight text-center">
								<div class="display-tc">
									<a onclick="return delCart('{{ $row->name }}')" href="/cart/del/{{ $row->rowId }}" class="closed"></a>
								</div>
							</div>
						</div>
						@endforeach
...
<script>
	function updateCart(rowId,qty){
		$.get('/cart/update/'+rowId+'/'+qty,function(data){
			if(data == 'success'){
				location.reload();
			}
			else{
	...
	function delCart(name){
		alert("Bạn có muốn xóa sản phẩm "+name+" không?") ;
	}
</script>
@endsection